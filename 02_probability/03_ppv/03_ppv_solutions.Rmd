...
2. **Simulation approach**. Rather than building a population based on expected values, a population can be simulated using the provided probabilities. This approach was introduced in the previous lab.
    ...
    The following code creates a simulated dataset of 100,000 individuals that each have a disease status and test result. Run the code then answer the following questions. There are comments in the code that correspond to the following questions; i.e., when answering part a), look for the comment \texttt{(part a)}.
...
```
    ...
    As the prevalence of breast cancer increases, PPV also increases. If more women have the disease, the chance of a positive test result being a true positive increases (and the chances of the result being a false positive decreases).
    ...
    The following \textsf{R} chunk demonstrates how \textsf{R} can be used as a calculator; the code calculates PPV through applying Bayes' rule.
...
```
    ...
    The relationship between PPV and prevalence has very important implications in breast cancer and in screening populations for diseases.  A paper published on February 9, 2017 (yes, just last week) in *Cancer Epidemiology Biomarkers \& Prevention* suggests that women who experience a false positive mammogram are less likely to return for their regularly scheduled mammogram than women who receive a true negative.  False positives are usually confirmed with a biopsy, and this unpleasant experience seems to have a negative influence on health behavior. You may be able to view the article through the Harvard Library online resources; it had not yet been posted on the morning of February 9.
...
```
    ...
    The effect on sensitivity can be seen by creating a table of a hypothetical population of 10,000 men. The table below corresponds to an overall prevalence of 0.05, sensitivity of 0.20, and specificty of 0.94. Note that since prevalence remains constant (in a given age group), the totals for $D$ and $D^C$ must remain the same.
    ...
    The following explanation is based on the idea that men with high PSA levels (i.e. between 4.1 ng/ml and 2.5 ng/ml) are far more likely to have prostate cancer. Thus, it is unlikely that an increase in positive tests will completely consist of false positives; it is also unlikely that all men who previously tested negative did not have prostate cancer.

    - Suppose the number of positive tests ($T^+$, Total) were to increase to 700. Unless all of the additional 30 positive tests are false positives, the 100 in the upper left cell ($T^+$, $D$) must increase, and the number of false negatives ($T^-$, $D$) must decrease (since the total $D$ must remain constant). This causes test sensitivity ($P(T^{+}|D)$), the ratio of the upper left cell to 500), to increase. 

    - The number of negative tests ($T^-$, Total) decreases, since a man's PSA level would now have to be below 2.5 ng/ml to be classified as testing negative. Unless all of the 30 previously negative tests were true negatives, the 8,930 in the middle cell ($T^-$, $D^C$) must decrease, and the number of false positives ($T^+$, $D^C$) must increase (since the total $D^C$ must remain constant). This causes test specificity ($P(T^{-}|D^C)$), the ratio of the middle cell to 9,500), to decrease. 

    \textit{Alternative Justification.} If the assumption is made that all additional positive tests are false positives, changing the 570 in the upper middle cell to 600, then the number of true negatives ($T^-$, $D^C$) decreases. This has no effect on test sensitivity and decreases test specificity.
