...
#define constants
x.bar = mean(prevend.sample$Age)
y.bar = mean(prevend.sample$RFFT)
s.x = sd(prevend.sample$Age)
s.y = sd(prevend.sample$RFFT)
r = cor(prevend.sample$Age, prevend.sample$RFFT)
...
plot(prevend.residuals ~ prevend.predicted, cex = 0.5,
     main = "Residual Plot for RFFT versus Age (n = 500)", xlab = "Predicted RFFT Score",
     ylab = "Residual")
...
plot(frog$clutch.volume ~ frog$body.size, 
     ...
     xlab = "Female body size (cm)", ylab = "Clutch volume (mm^3)")
...
plot(resid(frog.model) ~ predict(frog.model),
     main = "Residual Plot for Clutch Volume vs. Female Body Size",
     ylab = "Residual",
     xlab = "Predicted Clutch Volume (mm^3)",
     cex = 0.75)
abline(h = 0, col = "red", lty = 2)
```
