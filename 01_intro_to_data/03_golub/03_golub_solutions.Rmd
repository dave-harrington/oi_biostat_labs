...
```
    ...
    c) How many rows and columns does \texttt{large.out} have? Explain why.
        ...
        \textcolor{NavyBlue}{The matrix \texttt{large.out} has two columns and ten rows. It is constructed from all two columns of \texttt{diff.mean.expression.matrix}, and only the rows for which the expression value is greater than \texttt{ub.outlier}}
