...
Discussion with JV, 14 Jan 2019:

Try starting with a specific null and alternative, showing power, size.  Then ask the question about what happens when the alternative is one among many.  Or discuss research settings.  Ask a question about skepticism



--->

Suppose you read press reports in a reliable publication about two recently reported clinical trials.  One describes an advance in a disease in which there has been substantial progress in the last 5 years, such as the early stage breast cancer, using a treatment regimen that is a modification of something that has been successful.  The second summarizes an potentially important advance in a disease where recent studies have not led to any progress, such as alzheimers disease.  Both studies were well designed randomized trials with statistically significant results ($ p < 0.05$), when the new treatments were compared with currently used approaches.

A friend tells you she is more skeptical about the results of the second study but believes the first result.  Is her intution correct?  This lab explores how one might quantify that skepticism.  It begins by reviewing the concept of type I error and power.





This lab explores an important question of hypothesis testing that has not been examined earlier: how likely is it that the alternative hypothesis is true, given that a test is statistically significant?

The question is similar to the important question that arises in diagnostic testing: given that a diagnostic test is positive for the presence of a disease, how likely is it that the disease is in fact present.  Bayes' rule provided a method to answer this question.  Given that a patient in fact has an undiagnosed disease, the sensitivity of a diagnostic test is the conditional probability that the test is positive for the disease.  The positive predictive value of a diagnostic test is the reverse conditional probability - given that the test is positive, the positive predictive value is the  conditional probability that a disease is present.

Previous labs in this unit have emphasized the concept that hypothesis testing is used to establish evidence in favor of an alternative hypothesis.  A fundamental assumption in hypothesis testing is that either the null hypothesis or the alternative is true.  The investigator simply does not know which is the case.  Type I error probability is controlled by rejecting the null hypothesis only when the data would be extreme if the null hypothesis should be true.  The power of the test is controlled by choosing a large enough sample size so that, under a particular alternative hypothesis is true, the data will appear extreme under the null hypothesis with high probability.

In medical and life sciences research, the null

Outline of approach:

- Often, investigator asks how likely is it that the conclusion is true.  Reach for the analogy with diagnostic testing.

- Explain setting: there are fields where there has been little progress and others where progress is fast.

- Repeat power simulation for a fixed alternative.

- Simple setting showing distributions of alternatives
...
for(k in 1:num.iterations){
  ...
  if(hypothesis[k] == "null"){
    ...
    control = rnorm(n = 100, mean = 140, sd = 12)
    treatment = rnorm(n = 100, mean = 140, sd = 12)
  ...
  if(hypothesis[k] == "alternative"){
    ...
    control = rnorm(n = 100, mean = 140, sd = 12)
    treatment = rnorm(n = 100, mean = 137, sd = 12)
