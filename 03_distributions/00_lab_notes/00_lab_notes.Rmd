...
where \texttt{q} is $k$, \texttt{mean} is the parameter $\mu$, and \texttt{sd} is the parameter $\sigma$. By default, \textsf{R} calculates $P(X \leq k)$ and assumes that mean and standard deviation are 0 and 1, respectively. In order to compute $P(X > k)$, specify \texttt{lower.tail = FALSE}.

The following code show show to calculate $P(X \leq 105)$ and $P(X > 105)$ for $X \sim \text{N}(100, 5)$ and $P(Z \leq 1)$ and $P(Z > 1)$ for $Z \sim \text{N}(0, 1)$. 
...
Simulating geometric and negative binomial random variables requires the use of new programming control structures: the \texttt{repeat} and \texttt{while} loops.
