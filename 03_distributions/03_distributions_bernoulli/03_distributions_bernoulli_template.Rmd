...
1.  This question uses the scenario in Example 3.38 of the text. In the Milgram shock experiments, the probability of a person refusing to give the most severe shock is $p = 0.35$. Suppose that participants are tested one at a time until one person refuses.
    ...
    b) The \texttt{if-else} statement is closely related to the \texttt{if} statement and has the basic structure \texttt{if( condition ) \{ statement 1 \} else \{ statement 2 \}}. If the condition is satisfied, the first condition is carried out; else, the second statement is carried out.
...
3. In the Milgram shock experiments, what is the probability that five participants must be tested in order to observe four participants who refuse to administer the most severe shock? In other words, what is the probability of a fourth success on the fifth trial?
    ...
    d) Once a value can be recorded in \texttt{trials.for.rth.success} for the $k^{th}$ replicate of the experiment, why is it necessary to subtract \texttt{1} from \texttt{trial.number}? (*Hint*: It may help to think about the answer to parts b) and c).)
    ...
    g) Use \texttt{pnbinom()} to calculate the probability estimated in part f).
